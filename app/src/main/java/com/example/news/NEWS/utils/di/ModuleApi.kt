package com.example.news.NEWS.utils.di

import com.example.news.NEWS.data.server.NewsApiService
import com.example.news.NEWS.utils.BASE_URL
import com.example.news.NEWS.utils.TIMEOUT_TIME
...
object ModuleApi {
    ...
    @Provides
    @Singleton
    fun provideClient(time: Long) = OkHttpClient.Builder().connectTimeout(time,
        TimeUnit.SECONDS).readTimeout(time, TimeUnit.SECONDS)
